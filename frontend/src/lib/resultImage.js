...
const drawText = (context, text, x, y, maxWidth, maxHeight, options = {}) => {
  const { maximumSize = 32, minimumSize = 20, maximumLines = 5, weight = 600, lineHeightRatio = 1.16 } = options
  let size = maximumSize
  let lines = []
  let lineHeight = size * lineHeightRatio
  while (size >= minimumSize) {
    context.font = `${weight} ${size}px "Arha Magnit", Arial`
    lines = wrappedLines(context, text, maxWidth)
    lineHeight = size * lineHeightRatio
    if (lines.length <= maximumLines && size + Math.max(0, lines.length - 1) * lineHeight <= maxHeight) break
    size -= .5
  }
...
export async function createResultImage(inputText, translatedText) {
  ...
  const [title, star, logo, audience] = await Promise.all([
    loadSvg(asset('your-work2.svg'), (svg) => svg.replace(/<defs>[\s\S]*?<\/defs>/, '').replace(/<style>[\s\S]*?<\/style>/, '')),
  ...
  const sourceLines = drawText(context, inputText, -195, -15, 390, 108, { maximumSize: 34, minimumSize: 20, maximumLines: 4, weight: 400, lineHeightRatio: 1.12 })
  context.save(); context.translate(42, Math.min(18, -4 + sourceLines * 5)); context.scale(101 / 46, 85 / 47)
