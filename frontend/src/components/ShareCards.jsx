...
import { getSourceTextLayout, SOURCE_TEXT_LAYOUT } from '../lib/shareTextLayout.js'
...
function ShareCards({
  ...
  const sourceArrowRef = useRef(null)
  const resultPanelRef = useRef(null)
  const resultCopyRef = useRef(null)

  useLayoutEffect(() => {
    const targets = [
      [resultPanelRef.current, resultCopyRef.current],
    ].filter(([panel, copy]) => panel && copy)
    const sourceCopy = sourceCopyRef.current
    if (!targets.length && !sourceCopy) return undefined

    let animationFrame = 0
    const fitText = (panel, copy) => {
      ...
      const panelAvailableHeight = panel.clientHeight
        - Number.parseFloat(panelStyle.paddingTop)
        - Number.parseFloat(panelStyle.paddingBottom)
      if (availableWidth <= 0 || panelAvailableHeight <= 0) return

      // Те же пропорции, что используются в итоговом PNG:
      // введённый текст — максимум 34px на 390px полезной ширины,
      // результат — максимум 32px на 385px.
      const isSourceText = copy === sourceCopyRef.current
      const maximumSize = availableWidth * (isSourceText ? 34 / 390 : 32 / 385)
      const minimumSize = availableWidth * (isSourceText ? 4 / 390 : 6 / 385)
      const availableHeight = isSourceText
        ? Math.min(panelAvailableHeight, availableWidth * 108 / 390)
        : Math.min(panelAvailableHeight, availableWidth * 158 / 385)
    ...
    const fitAllText = () => {
      ...
      animationFrame = window.requestAnimationFrame(() => {
        if (sourceCopy && sourceValue) {
          ...
          const sourceArrow = sourceArrowRef.current
          if (sourcePanel && sourceArrow) {
            const panelStyle = window.getComputedStyle(sourcePanel)
            const availableWidth = sourcePanel.clientWidth
              - Number.parseFloat(panelStyle.paddingLeft)
              - Number.parseFloat(panelStyle.paddingRight)
            const sourceScale = availableWidth / SOURCE_TEXT_LAYOUT.width
            sourceCopy.style.fontSize = `${sourceLayout.size * sourceScale}px`
            const textHeight = (
              sourceLayout.size
              + Math.max(0, sourceLayout.lines.length - 1) * sourceLayout.lineHeight
            ) * sourceScale
            const arrowTop = Math.min(
              sourcePanel.clientHeight - sourceArrow.offsetHeight - 4,
              16 * sourceScale + textHeight,
            )
            sourceArrow.style.top = `${Math.max(4, arrowTop)}px`
          } else {
            sourceCopy.style.fontSize = `${sourceLayout.size}px`
          }
        }
    ...
    <div className="share-stack">
      <div className="share-stack__source">
        ...
        <p ref={sourcePanelRef} className="share-stack__panel share-stack__source-text">
          <span ref={sourceCopyRef} className={`share-stack__source-copy${sourceValue ? ' share-stack__source-copy--dynamic' : ''}`}>
            {sourceValue ? renderLines(sourceValue) : (
              <>
          ...
          <svg ref={sourceArrowRef} className="share-stack__source-arrow" viewBox="0 0 46 47" aria-hidden="true">
